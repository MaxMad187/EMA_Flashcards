...
@Entity(tableName = "repository_cards",
        primaryKeys = ["repoId", "cardId"],
        foreignKeys = [ForeignKey(entity = Repository::class, parentColumns = ["repoId"], childColumns = ["repoId"], onDelete = ForeignKey.CASCADE),
                       ForeignKey(entity = Flashcard::class , parentColumns = ["cardId"], childColumns = ["cardId"], onDelete = ForeignKey.CASCADE)
        ],
